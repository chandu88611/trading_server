...
class ZebuService {
    ...
    async requestTokenWithPasswordAndTotp(account, password, totp) {
        const meta = this.asObject(account?.accountMeta);
        const zebu = this.asObject(meta.zebu);
        const baseUrl = this.normalizeQuickAuthBaseUrl(this.pickFirstString(zebu.baseUrl, process.env.ZEBU_BASE_URL, "https://go.mynt.in/NorenWClientWeb"));
        ...
        for (const uidCandidate of uidCandidates) {
            ...
            for (const appkey of appkeyCandidates) {
                ...
                for (const variant of bodyVariants) {
                    ...
                    const retryable = emsg.includes("jdata is not valid json object") ||
