...
class Mt5ListenerDBServices {
    constructor(dataSource) {
        ...
        this.mt5SymbolRepo = this.dataSource.getRepository(entity_1.Mt5Symbol);
    }
    async ensureSchema() {
        const statements = [
            `
     ...
     */
    async findAccountByPollKey(brokerAccountId, pollKey) {
        if (!brokerAccountId || !pollKey || pollKey.length < 16)
            return false;
        const rows = await this.dataSource.query(`SELECT 1 FROM user_trading_accounts WHERE account_id = $1 AND mt5_poll_key = $2 LIMIT 1`, [brokerAccountId, pollKey]);
        return rows.length > 0;
    }
    /** Store MT5 funds (balance/equity/margin) into account_meta.mt5Funds by account_id. */
    async updateMt5Funds(brokerAccountId, funds) {
        await this.dataSource.query(`UPDATE user_trading_accounts
         SET account_meta = COALESCE(account_meta, '{}'::jsonb) || jsonb_build_object('mt5Funds', $2::jsonb)
       WHERE account_id = $1`, [brokerAccountId, JSON.stringify(funds)]);
    }
    /** Read MT5 funds for one of the user's accounts (ownership-checked). */
    async getMt5Funds(userId, tradingAccountId) {
        const rows = await this.dataSource.query(`SELECT account_meta->'mt5Funds' AS funds
         FROM user_trading_accounts
        WHERE id = $1 AND user_id = $2
        LIMIT 1`, [tradingAccountId, userId]);
    ...
    async getNextPendingJob(brokerAccountId) {
        return this.dataSource.transaction(async (manager) => {
            const rows = await manager.query(`SELECT ts.id, tss.status
         ...
         LIMIT 1
         FOR UPDATE OF tss SKIP LOCKED`, [brokerAccountId]);
            if (!rows.length)
                return null;
            const { id, status } = rows[0];
            const newStatus = status === "pending_close" ? "in_progress_close" : "in_progress";
            await manager.query(`UPDATE trade_signals_status SET status = $1 WHERE signal_id = $2`, [newStatus, id]);
            return manager.findOne(TradeSignals_1.TradeSignal, {
                where: { id },
     ...
     */
    async resetStaleInProgressJobs(staleAfterSeconds = 7) {
        const result = await this.dataSource.query(`UPDATE trade_signals_status
       ...
       AND updated_at < NOW() - make_interval(secs => $1::float)`, [staleAfterSeconds]);
        return result[1] ?? 0;
    }
    toBigIntText(value) {
    ...
    async markJobSuccess(job, refs) {
        const normalizedRefs = typeof refs === "object" && refs !== null ? refs : { orderId: refs };
    ...
    async markJobCloseSuccess(job, refs) {
        const normalizedRefs = typeof refs === "object" && refs !== null ? refs : { orderId: refs };
        const orderId = this.toBigIntText(normalizedRefs.orderId);
        const brokerOrderId = this.toBigIntText(normalizedRefs.brokerOrderId);
        const brokerPositionId = this.toBigIntText(normalizedRefs.brokerPositionId);
    ...
    async replaceMt5Symbols(brokerAccountId, items) {
        ...
        await this.mt5SymbolRepo.insert(items.map((item) => ({
            brokerAccountId: normalizedBrokerAccountId,
            symbol: String(item.symbol).trim().toUpperCase(),
            digits: item.digits === undefined || item.digits === null ? null : Number(item.digits),
            point: item.point === undefined || item.point === null ? null : String(item.point),
            tickSize: item.tickSize === undefined || item.tickSize === null ? null : String(item.tickSize),
            pipSize: item.pipSize === undefined || item.pipSize === null ? null : String(item.pipSize),
        })));
    }
    async getMt5SymbolMeta(brokerAccountId, symbol) {
        const normalizedBrokerAccountId = String(brokerAccountId ?? "").trim();
        const normalizedSymbol = String(symbol ?? "").trim().toUpperCase();
        if (!normalizedBrokerAccountId || !normalizedSymbol)
            return null;
