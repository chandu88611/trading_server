...
class Mt5ListenerServices {
    ...
    extractAckRefs(ack) {
        const orderId = this.toPositiveBrokerRef(ack?.orderId) ??
            this.toPositiveBrokerRef(ack?.order_id) ??
            this.toPositiveBrokerRef(ack?.ticket);
        const brokerOrderId = this.toPositiveBrokerRef(ack?.brokerOrderId) ??
            this.toPositiveBrokerRef(ack?.broker_order_id) ??
            this.toPositiveBrokerRef(ack?.orderId) ??
            this.toPositiveBrokerRef(ack?.order_id);
        const brokerPositionId = this.toPositiveBrokerRef(ack?.brokerPositionId) ??
    ...
    async getSignalForEA(brokerAccountId) {
        try {
            ...
            try {
                ...
                const needsDistanceMapping = stopLossDistance !== undefined || takeProfitDistance !== undefined;
                ...
                await this.dbService.markJobFailed(job, error instanceof Error ? error.message : String(error));
                return {};
            }
            return payload;
        }
    ...
    async handleState(brokerAccountId, payload) {
        const normalizedBrokerAccountId = this.firstNonEmptyString(brokerAccountId, payload?.brokerAccountId, payload?.userId, payload?.account_login, payload?.accountLogin);
        if (!normalizedBrokerAccountId) {
            return { ok: false, error: "broker_account_id_required" };
        }
        const hasSymbolItems = Array.isArray(payload?.items) ||
        ...
        const items = itemsRaw
            ...
            point: item?.point === undefined || item?.point === null ? null : Number(item.point),
            tickSize: item?.tickSize === undefined || item?.tickSize === null
                ? item?.tick_size === undefined || item?.tick_size === null
                    ? null
                    : Number(item.tick_size)
                : Number(item.tickSize),
            pipSize: item?.pipSize === undefined || item?.pipSize === null
    ...
    async handleAck(ack) {
        ...
        if (status === "success") {
            if (isCloseJob) {
                await this.dbService.markJobCloseSuccess(job, refs);
                return;
            }
            await this.dbService.markJobSuccess(job, refs);
            return;
        }
