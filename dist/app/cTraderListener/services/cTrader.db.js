...
class CTradeSignalDB {
    constructor() {
        ...
        this.cTraderSymbolRepo = data_source_1.default.getRepository(CTraderSymbol_1.CTraderSymbol);
    }
    async ensureSchema() {
        const statements = [
    ...
    computeRetryAt(currentAttempts, retryBaseDelayMs) {
    ...
    async scheduleJobRetry(job, nextStatus, error, maxRetryAttempts, retryBaseDelayMs) {
    ...
    async getAccountForFunds(userId, tradingAccountId) {
        const acc = await this.tradingAccountRepo.findOne({
            where: { id: tradingAccountId, userId },
        });
        if (!acc)
            return null;
        const env = String(acc?.accountMeta?.env ?? "demo").trim().toLowerCase();
        return {
            ...
            refreshToken: String(acc.refreshToken ?? "").trim() || undefined,
        };
    }
    ...
    async getCTraderSymbolMeta(userId, env, accountId, symbol) {
        ...
        try {
            ...
            for (const s of ordered) {
                ...
                structured.push({
                    ...
                    executionMode: s.executionMode === "OPEN" || s.executionMode === "AMEND_SLTP"
                        ? s.executionMode
                        : null,
                    ...
                    trailingTakeProfitActivationDistance: s.trailingTakeProfitActivationDistance ?? null,
    ...
    async updateTradeStatus(data) {
        ...
        await Promise.all(data.map(async (item) => {
            ...
            if (status === "retry_pending" || status === "retry_pending_close") {
                await this.scheduleJobRetry(job, status === "retry_pending_close" ? "pending_close" : "pending", item.error ?? "execution_retry_scheduled", Number(item.maxRetryAttempts ?? 5), Number(item.retryBaseDelayMs ?? 1000));
                return;
            }
    ...
    async findLatestOpenSignalByEntryRef(tradingAccountId, entryRef, excludeSignalId) {
    ...
    async markSignalClosedById(tradeSignalId) {
