...
async function initKite() {
    ...
    if (requestToken && apiSecret) {
        ...
        console.log("Kite: request token or api secret not provided — skipping generateSession");
    }
    try {
        const margins = await kc.getMargins();
        console.log("Kite: margins fetched", margins && typeof margins === "object" ? "ok" : "no-data");
    }
    catch (err) {
