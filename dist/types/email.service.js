...
exports.sendCopyTradingRequestEmail = sendCopyTradingRequestEmail;
exports.sendSupportTicketReplyEmail = sendSupportTicketReplyEmail;
exports.sendTradeNotificationEmail = sendTradeNotificationEmail;
...
async function sendCopyTradingRequestEmail({ masterEmail, masterName, followerName, followerEmail, requestId, }) {
    if (!mail_1.isMailEnabled)
        return;
    const baseUrl = process.env.APP_BASE_URL;
    if (!baseUrl) {
...
async function sendSupportTicketReplyEmail({ email, ticketSubject, replyBody, executiveName, }) {
    if (!mail_1.isMailEnabled)
        return;
    await mail_1.mailTransporter.sendMail({
        from: process.env.SMTP_FROM,
...
async function sendTradeNotificationEmail(params) {
    if (!mail_1.isMailEnabled)
        return;
    const { userEmail, event, symbol, exchange, side, quantity, price, brokerOrderId, errorMessage, pnl, broker, tradeSignalId, } = params;
    const subjectMap = {
    ...
    const rows = [
        ...
        .filter((r) => r !== null)
        .map(([label, value]) => `<tr><td style="padding:6px 12px;color:#94a3b8;white-space:nowrap">${label}</td><td style="padding:6px 12px;color:#f1f5f9;font-weight:600">${value}</td></tr>`)
        .join("");
    const html = `
