...
export class DeltaController {
	private service = new DeltaService();
	...
	async saveToken(req: Request, res: Response) {
		...
		return res.status(200).json({
			message: "delta_credentials_saved",
			data: result,
		});
	}
	...
	async deleteToken(req: Request, res: Response) {
		...
		return res.status(200).json({
			message: "delta_credentials_deleted",
			data,
		});
	}
...
@ControllerError()
async publicProducts(_req: Request, res: Response) {
	const data = await this.service.getPublicProducts();
	...
	async publicTicker(req: Request, res: Response) {
		...
		const productIdRaw = Number((req.query as any)?.productId ?? undefined);
		const productId = Number.isFinite(productIdRaw) ? productIdRaw : undefined;

		const data = await this.service.getPublicTicker({
			symbol,
			productId,
		});
	...
	async publicOrderbook(req: Request, res: Response) {
		...
		if (!symbol && !productId) {
			return res.status(HttpStatusCode._BAD_REQUEST).json({
				message: "symbol_or_productId_required",
			});
		}

		const data = await this.service.getPublicOrderbook({
			symbol,
			productId,
		});
