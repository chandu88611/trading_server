...
function normalizeCapitalRequirement(value: unknown) {
...
function normalizeDefaultParams(value: unknown) {
  let parsed = value;
  if (typeof value === "string") {
    try {
...
function mapDuplicateStrategyCodeError(error: any): never {
  if (error?.code === "23505") {
    throw {
...
export class StrategyDBService {
  ...
  private userStrategyRepo: Repository<UserStrategyInstance>;

  constructor() {
    this.repo = AppDataSource.getRepository(Strategy);
    this.userStrategyRepo = AppDataSource.getRepository(UserStrategyInstance);
  }
  ...
  async create(payload: any) {
    if (!payload?.name?.trim()) throw badRequest("name required");
    if (!payload?.category?.trim()) throw badRequest("category required");

    const strategyCode = normalizeStrategyCode(
      payload.strategyCode ?? payload.strategy_code ?? payload.name
    );
    ...
    const row = this.repo.create({
      ...
      version:
        payload.version === undefined ? 1 : normalizeVersion(payload.version),
      defaultParams:
        payload.defaultParams !== undefined || payload.default_params !== undefined
      ...
      capitalRequirement: normalizeCapitalRequirement(
        payload.capitalRequirement ?? payload.capital_requirement
      ),
      isActive:
      ...
      isDeprecated:
        payload.isDeprecated === undefined && payload.is_deprecated === undefined
          ? false
          : normalizeBoolean(
              payload.isDeprecated ?? payload.is_deprecated,
              "isDeprecated"
            ),
      isCopyable:
        payload.isCopyable === undefined && payload.is_copyable === undefined
          ? true
  ...
  async list(query: StrategyListQuery) {
    ...
    if (query.searchParam?.trim()) {
      qb.andWhere(
        "(strategy.name ILIKE :search OR strategy.description ILIKE :search OR strategy.strategy_code ILIKE :search)",
        { search: `%${query.searchParam.trim()}%` }
      );
    }
  ...
  async getById(strategyId: number, options?: { availableOnly?: boolean }) {
  ...
  async getDetail(
    strategyId: number,
    options?: { availableOnly?: boolean; userId?: number | null }
  ) {
    ...
    return {
      ...strategy,
      provider: (strategy.defaultParams as any)?.provider ?? null,
      riskLevel: strategy.riskProfile ?? (strategy.defaultParams as any)?.riskLevel ?? null,
      ...
      equityCurve: equityCurve.map((row: any) => ({
        date: row.date,
  ...
  private async getCurrentUserInstance(userId: number, strategyId: number) {
  ...
  private async ensureTradingAccount(userId: number, tradingAccountId: number | null) {
    ...
    if (!rows[0]) {
      throw { statusCode: HttpStatusCode._BAD_REQUEST, message: "invalid_trading_account" };
    }
    return Number(rows[0].id);
  }

  private normalizeCopySettings(payload: any) {
  ...
  private normalizeVolume(payload: any, fallback: unknown) {
    ...
    if (!Number.isFinite(volume) || volume < 0.01 || volume > 99.99) {
      throw { statusCode: HttpStatusCode._BAD_REQUEST, message: "volume_must_be_between_0.01_and_99.99" };
    }
    return volume.toFixed(2);
  }

  async subscribe(userId: number, strategyId: number, payload: any) {
    ...
    const tradingAccountId =
      payload.accountId ?? payload.tradingAccountId ?? payload.trading_account_id;
    const validatedAccountId = await this.ensureTradingAccount(
      userId,
      tradingAccountId === undefined || tradingAccountId === null || tradingAccountId === ""
        ? null
  ...
  async getMyPerformance(userId: number, strategyId: number, accountId?: number | null) {
    if (!Number.isFinite(userId) || userId <= 0) {
      throw { statusCode: HttpStatusCode._UNAUTHORISED, message: "user_not_authorized" };
    }
    await this.getById(strategyId, { availableOnly: true });
  ...
  async update(strategyId: number, payload: any) {
    if (!Number.isFinite(strategyId) || strategyId <= 0) {
      throw { statusCode: HttpStatusCode._BAD_REQUEST, message: "invalid_strategy_id" };
    }
    ...
    const hasMutableField = Object.keys(payload).some((key) =>
      STRATEGY_MUTABLE_FIELDS.has(key)
    );
    if (!hasMutableField) {
    ...
    if (payload.strategyCode !== undefined || payload.strategy_code !== undefined) {
      strategy.strategyCode = normalizeStrategyCode(
        payload.strategyCode ?? payload.strategy_code
      );
    }
    if (payload.name !== undefined) {
      if (!String(payload.name).trim()) throw badRequest("name cannot be empty");
    ...
    if (payload.defaultParams !== undefined || payload.default_params !== undefined) {
      strategy.defaultParams = normalizeDefaultParams(
        payload.defaultParams ?? payload.default_params
      );
    }
    if (
    ...
    if (
      payload.capitalRequirement !== undefined ||
      payload.capital_requirement !== undefined
    ) {
      strategy.capitalRequirement = normalizeCapitalRequirement(
        payload.capitalRequirement ?? payload.capital_requirement
      );
    }
    if (payload.isActive !== undefined || payload.is_active !== undefined) {
      strategy.isActive = normalizeBoolean(
        payload.isActive ?? payload.is_active,
        "isActive"
      );
    }
    if (
      payload.isDeprecated !== undefined ||
      payload.is_deprecated !== undefined
    ) {
      strategy.isDeprecated = normalizeBoolean(
        payload.isDeprecated ?? payload.is_deprecated,
        "isDeprecated"
      );
    }
    if (payload.isCopyable !== undefined || payload.is_copyable !== undefined) {
      strategy.isCopyable = normalizeBoolean(
        payload.isCopyable ?? payload.is_copyable,
        "isCopyable"
      );
    }
  ...
  async retire(strategyId: number) {
    if (!Number.isFinite(strategyId) || strategyId <= 0) {
      throw { statusCode: HttpStatusCode._BAD_REQUEST, message: "invalid_strategy_id" };
    }
  ...
  async setUserStrategyInstanceStatus(
    ...
    instance.status =
      status === "active" ? UserStrategyStatus.ACTIVE : UserStrategyStatus.PAUSED;
  ...
  async setUserStrategyStatusByStrategyId(
    userId: number,
    strategyId: number,
    status: "active" | "paused"
  ) {
  ...
  async setUserStrategyInstanceVolume(
    userId: number,
    instanceId: number,
    volume: number
  ) {
