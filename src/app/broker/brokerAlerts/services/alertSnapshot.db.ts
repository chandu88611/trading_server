...
import { AdminHistoryQuery, HistoryQuery, ICreateAlertSnapshot } from "../interfaces/alertSnapshot.interface";
...
import {
  AdminStrategyTrade,
  AdminStrategyTradeStatus,
} from "../../../../entity/AdminStrategyTrade";
...
export type CreateAdminStrategyTradePayload = {
  planId: number;
  strategyId: number;
...
export class AlertSnapshotDB {
  ...
  async createAdminStrategyTrade(
    payload: CreateAdminStrategyTradePayload,
    queryRunner: QueryRunner
  ) {
    const entity = queryRunner.manager.getRepository(AdminStrategyTrade).create({
    ...
    return queryRunner.manager.getRepository(AdminStrategyTrade).save(entity);
  }
  ...
  async getBrokerIdByCodes(marketType: MarketType, codes: string[]) {
  ...
  async closeOppositeCompletedTrades(
    {
      ...
      symbol: string;
      exchange?: string;
      incomingAction: "BUY" | "SELL";
    },
    queryRunner: QueryRunner,
  ): Promise<number> {
    if (!tradingAccountIds.length || !symbol?.trim()) return 0;

    const oppositeAction = incomingAction === "BUY" ? "SELL" : "BUY";

    const signalIds: { id: number }[] = await queryRunner.manager
      ...
      .andWhere(
        "((ts.orderId IS NOT NULL AND ts.orderId > 0) OR (ts.brokerOrderId IS NOT NULL AND ts.brokerOrderId > 0) OR (ts.brokerPositionId IS NOT NULL AND ts.brokerPositionId > 0))"
      )
