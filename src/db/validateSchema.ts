...
async function tableExists(
  ...
  for (const name of candidates) {
    try {
      ...
      if (res && res[0] && res[0].r) return true;
    } catch (err) {
      // ignore and try next
    }
  }
  ...
  try {
    ...
    if (r && r[0] && (r[0].exists === true || r[0].exists === "t")) return true;
  } catch (err) {
    // ignore
  }
  return false;
...
export async function validateSchema(
  ds: DataSource,
  options?: { requiredTables?: string[] }
) {
  const missing: string[] = [];
  const tables =
    options?.requiredTables ?? ds.entityMetadatas.map((m) => m.tableName);
  ...
  if (missing.length) {
    throw new Error(
      `Database schema validation failed — missing tables: ${missing.join(
        ", "
      )}`
    );
  }
